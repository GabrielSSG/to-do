...
type TaskListProps = {
    data: Task[];
    changeTaskState: (id: number) => void;
    removeTask: (id: number) => void;

}

export function TaskList({ data, changeTaskState, removeTask }: TaskListProps) {
    ...
    return (
        <ul className={style.container}>
            {data.length ?
                data.map((task) => {
                    return (
                        <div key={task.id} className={classNames(style.task, task.done && style.taskDone)}>
                            <input 
                                type="checkbox" 
                                checked={task.done} 
                                onChange={() => changeTaskState(task.id)} 
                            />
                            <span>{task.description}</span>   
                            <button type="button" onClick={() => removeTask(task.id)}>
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M14.7021 9.98547H13.3716V15.5073H14.7021V9.98547Z" />
                                    <path d="M11.9624 9.98547H10.6318V15.5073H11.9624V9.98547Z" />
                ...
                : <div className={style.emptyMessage}>
                    <img src={Clipboard} alt="Icone" className={style.icon} />
